...
async function isAdmin(adminId) {
  ...
  const [rows] = await db.execute(
    'SELECT id FROM users WHERE id = ? AND adm = 1 AND ativo = 1 LIMIT 1',
    [adminId]
  );
  ...
  try {
    ...
    const [rows] = await db.execute(
      `SELECT id, nome_completo AS nome, numero_oab AS oab, cpf, email,
         ...
         FROM users WHERE ativo = 0 ORDER BY criado_em DESC`
    );
...
async function approveUser(req, res) {
  try {
    ...
    if (body.senha) {
      await db.execute(
        'UPDATE users SET ativo = 1, senha_hash = SHA2(?, 256), atualizado_em = NOW() WHERE id = ? AND ativo = 0',
        [body.senha, body.userId]
      );
    } else {
      await db.execute(
        'UPDATE users SET ativo = 1, atualizado_em = NOW() WHERE id = ? AND ativo = 0',
        [body.userId]
      );
    }
...
async function rejectUser(req, res) {
  try {
    ...
    await db.execute('DELETE FROM users WHERE id = ? AND ativo = 0', [body.userId]);
    json(res, 200, { success: true });
  } catch (err) {
...
async function editUser(req, res) {
  try {
    ...
    if (body.senha) {
      await db.execute(
        `UPDATE users SET nome_completo=?, email=?, cpf=?, numero_oab=?, nome_usuario=?,
...
async function createUser(req, res) {
  try {
    ...
    const [emailExists] = await db.execute('SELECT id FROM users WHERE email=? LIMIT 1', [email]);
    if (emailExists.length > 0) return json(res, 409, { error: 'E-mail ja cadastrado.' });

    if (oab) {
      const [oabExists] = await db.execute('SELECT id FROM users WHERE numero_oab=? LIMIT 1', [oab]);
      if (oabExists.length > 0) return json(res, 409, { error: 'Numero OAB ja cadastrado.' });
    }

    if (usuario) {
      const [uExists] = await db.execute('SELECT id FROM users WHERE nome_usuario=? LIMIT 1', [usuario]);
      if (uExists.length > 0) return json(res, 409, { error: 'Usuario ja cadastrado.' });
    }

    await db.execute(
      `INSERT INTO users (nome_completo, numero_oab, cpf, email, nome_usuario, senha_hash, ativo, criado_em, atualizado_em)
